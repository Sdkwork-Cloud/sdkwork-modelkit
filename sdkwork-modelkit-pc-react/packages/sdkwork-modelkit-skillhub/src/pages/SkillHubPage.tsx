...
import { PublishSkillModal } from './modals/PublishSkillModal';
import { InstallSkillModal } from './modals/InstallSkillModal';
...
export function SkillHubPage() {
  ...
  useEffect(() => {
    ...
    skillhubService.getSkills().then(setSkillsList);
    skillhubService.getCategories().then(setCategories);
  }, []);

  const sortOptions = ['Featured', 'Most downloaded', 'Most starred', 'Most installed', 'Recently updated', 'Newest', 'Name'];

  const filteredSkills = useMemo(() => {
    let result = skillsList.filter(skill => {
      const matchCat = selectedCategory === 'All' ? true : selectedCategory === 'Installed' ? skill.installedAgents.length > 0 : skill.type === selectedCategory;
      const matchAgent = selectedFilterAgent === 'All' ? true : skill.installedAgents.includes(selectedFilterAgent);
      const matchSearch = skill.name.toLowerCase().includes(searchQuery.toLowerCase()) || skill.desc.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div className="flex flex-1 h-full bg-canvas text-text-main font-sans overflow-hidden">
      ...
      <div className="w-[220px] border-r border-divider bg-panel flex flex-col py-5 shrink-0 h-full">
        ...
        <div className="relative px-3 mb-5">
          ...
          <input 
            type="text" 
            placeholder={t('skillhub:search_skills')} 
            value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col p-6 lg:p-8">
          ...
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-[24px] font-semibold tracking-tight text-text-main flex items-baseline gap-3">
              {selectedCategory === 'All' ? 'All Skills' : t(selectedCategory, selectedCategory)} <span className="text-sm text-text-muted font-normal">{filteredSkills.length} found</span>
            </h1>
          </div>
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 bg-surface/50 p-4 rounded-xl border border-divider">
            <div className="flex items-center gap-2 overflow-x-auto stylish-scrollbar pb-1 xl:pb-0 hide-scrollbar-on-desktop">
              <div className="flex items-center gap-2">
                ...
                >
                  <option value="All">{t('skillhub:all_agents')}</option>
                  {agents.map(agent => (
                ...
                {filteredSkills.map(skill => (
                  ...
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 truncate">
                          <div className="flex items-baseline gap-1.5">
                            <span className="text-text-muted text-xs font-medium">{skill.author}</span>
                            <span className="text-text-muted text-xs">/</span>
                            <h3 className="text-[15px] font-bold text-text-main group-hover:text-primary-light transition-colors truncate">{skill.name}</h3>
                          </div>
                          {skill.installedAgents.length > 0 && (
                            <div className="flex items-center gap-1.5 shrink-0 group/badge relative ml-1">
                              <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] uppercase font-bold tracking-wider flex items-center gap-1 cursor-default">
                                <CheckCircle2 size={10} /> {skill.installedAgents.length} Installed
                              </span>
                        ...
                        <div className="flex items-center gap-5 text-[11px] text-text-muted font-medium">
                          <span>{t('skillhub:updated')} {skill.updated}</span>
                          <span className="flex items-center gap-1.5"><Star size={12} className="text-text-muted" /> {skill.rating}k</span>
                          <span className="flex items-center gap-1.5"><Download size={12} className="text-text-muted" /> {skill.downloads}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={isGrid ? 'mt-4 pt-4 border-t border-divider flex justify-end' : 'absolute right-5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity'}>
                      ...
                      >
                        {skill.installedAgents.length > 0 ? t('skillhub:manage') : t('skillhub:install')}
                      </button>
                    </div>
                  </div>
                ))}
      ...
      <InstallSkillModal
        ...
        skillName={skillsList.find(s => s.id === installModalSkill)?.name || ''}
        agents={agents}
        initialSelectedAgents={installModalSkill !== null ? agents.filter(a => skillsList.find(s => s.id === installModalSkill)?.installedAgents.includes(a.name)).map(a => a.id) : []}
        onClose={() => setInstallModalSkill(null)}
        onConfirm={(selectedAgents) => {
          setInstallModalSkill(null);
          toast.success(`Skill installed to ${selectedAgents.length} agents`);
        }}
      />
      ...
      <PublishSkillModal
        ...
        onSkillPublished={(newSkill) => setSkillsList([newSkill, ...skillsList])}
      />
    </div>
  );
}