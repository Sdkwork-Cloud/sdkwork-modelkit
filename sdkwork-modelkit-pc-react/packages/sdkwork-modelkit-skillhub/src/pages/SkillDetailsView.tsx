...
interface SkillDetailsViewProps {
  ...
  agents: AgentTool[];
  onBack: () => void;
  onInstallClick: (skill: SkillItem) => void;
  renderIcon: (iconName: string, className?: string) => React.ReactNode;
}

export function SkillDetailsView({ skill, agents, onBack, onInstallClick, renderIcon }: SkillDetailsViewProps) {
  ...
  const getInstallCommand = () => {
    ...
    if (installTab === 'agent') {
      if (agentTool === 'openclaw') {
        return `Find via clawhub clawhub://skills/${skill.name}@${version} skill, using registry ${registry}。`;
      } else {
        return `Find via codex codex://skills/${skill.name}@${version} skill, using registry ${registry.replace('clawhub.io', 'codex.io')}。`;
      }
    }
    if (installTab === 'npm') return `npm install ${skill.name}@${version}${npmRegistry}`;
    if (installTab === 'pnpm') return `pnpm add ${skill.name}@${version}${npmRegistry}`;
    if (installTab === 'bun') return `bun add ${skill.name}@${version}${npmRegistry}`;
    return '';
  };
  ...
  return (
    <div className="w-full mx-auto p-6 lg:p-8">
      ...
      >
        ...
        <span className="font-medium">{t('skillhub:back_to_skills')}</span>
      </button>
      ...
      <div className="flex items-start justify-between gap-8 mb-12">
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 rounded-2xl bg-panel border border-divider flex items-center justify-center shrink-0 shadow-lg">
            {renderIcon(skill.icon, "text-emerald-500 w-10 h-10")}
          </div>
          <div>
            <h1 className="text-4xl font-bold text-text-main mb-3 tracking-tight">{skill.name}</h1>
            <p className="text-lg text-text-muted font-medium">{t('skillhub:by')} <span className="text-text-main">{skill.author}</span></p>
          </div>
        </div>
        <button 
        ...
        >
          <Plug size={20} />
          {skill.installedAgents.length > 0 ? t('skillhub:manage_integration') : t('skillhub:install_skill')}
        </button>
      </div>
      ...
      <div className="flex items-center gap-12 mb-14 text-sm text-text-muted border-y border-divider py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-text-main text-lg font-bold">{skill.rating}k</div>
            <div>Stars</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-text-main text-lg font-bold">{skill.downloads}</div>
            <div>Downloads</div>
          </div>
        </div>
        <div className="w-px h-10 bg-surface-hover"></div>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-text-main text-lg font-bold uppercase">{skill.type}</div>
            <div>Category</div>
          </div>
        </div>
      </div>
      ...
      {/* Installation Banner */}
      <div className="mb-12">
        <h3 className="text-sm font-medium text-text-main mb-4">{t("skillhub:search_desc", "Search skills. Supports version control, rollback anytime.")}</h3>
        <div className="bg-surface border border-divider rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-5 flex-wrap gap-4">
            <div className="flex items-center gap-4 text-xs font-semibold text-text-main">
              <span>{t("skillhub:one_click_install", "One-click install any skill package:")}</span>
              
              {installTab === 'agent' && (
                <div className="flex items-center gap-2 bg-canvas px-3 py-1.5 rounded-lg border border-divider">
                  <span className="text-text-muted">{t("skillhub:tool_label", "Tool:")}</span>
                  <select 
                    value={agentTool}
              ...
              <div className="flex items-center gap-2 bg-canvas px-3 py-1.5 rounded-lg border border-divider">
                <span className="text-text-muted">{t("skillhub:mirror_label", "Mirror:")}</span>
                <select 
                  value={mirror}
                  onChange={e => setMirror(e.target.value as any)}
                  className="bg-transparent outline-none cursor-pointer"
                >
                  <option value="official">{t("skillhub:official_default", "Official Default")}</option>
                  <option value="taobao">{t("skillhub:taobao_mirror", "Taobao Mirror")}</option>
                </select>
              </div>
            </div>
            
            <div className="flex bg-canvas border border-divider p-1 rounded-full text-xs font-bold">
              <button onClick={() => setInstallTab('agent')} className={`px-5 py-1.5 rounded-full transition-all ${installTab === 'agent' ? 'bg-[#ff6b4a] text-white shadow-sm' : 'text-text-muted hover:text-text-main hover:bg-surface-hover'}`}>{t("skillhub:agent_tab", "Agent")}</button>
          ...
          <div className="group relative bg-canvas border border-divider-strong rounded-xl p-4 font-mono text-[13px] text-text-main/80 flex items-center justify-between">
            <span className="break-all pr-8">
              {installTab === 'agent' ? (
                <>
                  {t("skillhub:find_via", "Find via")} {agentTool} {agentTool}://skills/{skill.name}@1.0.0 skill, using registry <span className="bg-surface px-1.5 py-0.5 rounded border border-divider text-text-main">{mirror === 'official' ? (agentTool === 'openclaw' ? 'https://registry.clawhub.io' : 'https://registry.codex.io') : 'https://registry.npmmirror.com'}</span>.
                </>
              ) : (
                getInstallCommand()
              )}
      ...
      <div className="grid grid-cols-3 gap-12">
        <div className="col-span-2 space-y-12">
          <section>
            ...
            <p className="text-[16px] text-text-main leading-relaxed font-medium">{skill.desc}</p>
          </section>
          ...
          <div className="bg-surface border border-divider rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-5">
              <div className="flex flex-col gap-2">
                ...
                <span className="text-text-main font-mono text-sm bg-surface px-3 py-1.5 rounded-lg inline-block w-fit border border-divider">{skill.schemaType}</span>
              </div>
              <div className="flex flex-col gap-2 pt-2 border-t border-divider">
                ...
                <span className="text-text-main font-medium text-sm flex items-center gap-2 bg-surface px-3 py-1.5 rounded-lg w-fit border border-divider">
                  <Key size={14} className="text-amber-400"/> {skill.authType}
                </span>
              </div>
              {/* @ts-ignore */}
              {skill.sourceType && (
                <div className="flex flex-col gap-2 pt-2 border-t border-divider">
                  <span className="text-text-muted text-xs font-semibold uppercase tracking-wider">{t('skillhub:txt_1145')}</span>
                  {/* @ts-ignore */}
                  {skill.sourceType === 'repo' ? (
                    <a 
                      /* @ts-ignore */
                      href={skill.repoUrl} 
                      target="_blank" 
                    ...
                    <span className="text-text-main font-medium text-xs flex items-center gap-1.5 bg-surface px-3 py-1.5 rounded-lg w-fit border border-divider">
                      {/* @ts-ignore */}
                      <FolderArchive size={13} className="text-emerald-400"/> {skill.zipFileName}
                    </span>
                  )}
                </div>
              )}
