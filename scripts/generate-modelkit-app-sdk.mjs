...
function parseArgs(argv) {
  const options = {
    dryRun: false,
    languages: [],
    sdkFamily: 'sdkwork-modelkit-app-sdk',
  };
...
function assertSdkgenEntrypoint(sdkgenEntrypoint) {
  if (!sdkgenEntrypoint || !fs.existsSync(sdkgenEntrypoint)) {
    throw new Error(
...
function createGenerationPlans(rootDir, options) {
  ...
  assert.equal(assembly.sdkOwner, 'sdkwork-modelkit');
  ...
  for (const language of requestedLanguages) {
    if (!SUPPORTED_LANGUAGES.has(language)) {
      throw new Error(`Unsupported Modelkit standard SDK language: ${language}.`);
    }
  }
...
export function generateModelkitAppSdk(options = {}, rootDir = process.cwd()) {
  const sdkgenEntrypoint = resolveSdkgenEntrypoint(rootDir);
  if (!sdkgenEntrypoint) {
    console.warn(
      '[sdkwork-modelkit] sdkgen unavailable; OpenAPI authority validated, hand-written preference client remains active.',
    );
    return [];
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    const result = generateModelkitAppSdk(parseArgs(process.argv.slice(2)));
